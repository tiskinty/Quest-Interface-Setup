...
import com.medplus.orders.OrderRequest;
import com.medplus.orders.OrderResponse;
import com.medplus.orders.OrderSubmissionPortType;
import com.medplus.orders.OrderSubmissionService;

/**
 * OrderSubmissionServiceClient submits lab order (HL7 messages) to the MedPlus Hub
 * platform.  Encapsulates the sending of an HL7 order to a Quest Lab
 * via the Hub's SOAP Web service via the JAX-WS framework.
 * 
 */
public class OrderSubmissionServiceClient {
    ...
    private String receivingFacility = "THO";
    ...
    public static void main(String[] args) {
        try {
            new OrderSubmissionServiceClient().sendOrder();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    
    public OrderSubmissionServiceClient() {
    ...
    private void sendOrder() {
        ...
        try {
            ...
            OrderRequest submitOrderRequest = getWebServiceOrder();
            OrderResponse response = port.submitOrder(submitOrderRequest);
            ...
            if (e.getCause() instanceof SOAPFaultException) {
                ...
                System.out.println(sfe.getFault().getFaultString());
            }
            e.printStackTrace();
        }
    }
    ...
    /**
     * getWebServiceOrder: construct the WebService request object using member
     * data
     * @return
     */
    private OrderRequest getWebServiceOrder() {
        OrderRequest request = new ObjectFactory().createOrderRequest();
